...
ModbusTcpClient::ModbusTcpClient(const std::string &host, uint16_t port, uint8_t unit_id, uint32_t timeout_ms)
...
bool ModbusTcpClient::send_request_(const std::vector<uint8_t> &pdu) {
  if (!this->connect_())
    return false;
  ...
  uint16_t length = static_cast<uint16_t>(pdu.size() + 1);  // unit id + pdu
...
bool ModbusTcpClient::read_response_(std::vector<uint8_t> &pdu_out) {
  uint8_t mbap[7];
...
bool ModbusTcpClient::read_holding_registers(uint16_t start_address, uint16_t count, std::vector<uint16_t> &out) {
  if (count == 0)
    return false;
